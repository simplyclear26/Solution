...
function StartInner() {
  ...
  if (status === 'validating') return (<div className="min-h-screen flex items-center justify-center bg-white"><div className="text-center"><div className="flex justify-center gap-2 mb-6">{[0,1,2].map(i => <div key={i} className="w-2 h-2 rounded-full bg-[#2AB8A0] animate-pulse" style={{ animationDelay: `${i*0.2}s` }} />)}</div><p className="text-[#636366] font-light text-sm">Verifying your access...</p></div></div>)
...
export default function StartPage() {
  return (<Suspense fallback={<div className="min-h-screen bg-white" />}><StartInner /></Suspense>)
}
