...
export function ModuleSection({ module, moduleIndex, totalModules, sectionNumber, totalSections, overallProgress, existingAnswers, onComplete, onBack, currentSection, completedSections }: ModuleSectionProps) {
  ...
  function goBack() {
    if (current > 0) { setDirection(-1); setCurrent(c => c - 1) } else { onBack() }
  }
  ...
  if (showIntro) {
    return (
      <div className="min-h-screen flex items-center justify-center px-6 py-16 pb-28">
        <div className="max-w-2xl w-full">
          <div className="mb-12">
            ...
            <div className="h-[3px] bg-[#E5E3DF] rounded-full overflow-hidden">
              <div className="h-full bg-[#2AB8A0] rounded-full transition-all duration-500" style={{ width: `${overallProgress}%` }} />
            </div>
          </div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-xs font-medium tracking-widest uppercase text-[#2AB8A0] mb-3">Module {moduleIndex + 1} of {totalModules}</p>
            <h2 className="font-serif text-[clamp(1.8rem,4vw,2.8rem)] text-[#1C1C1E] tracking-tight mb-4">{module.label}</h2>
            <p className="text-[#636366] font-light text-lg leading-relaxed mb-10">{MODULE_INTROS[module.id] ?? ''}</p>
            <p className="text-sm text-[#8E8E93]">{module.steps.length} questions in this module</p>
            <div className="flex gap-4 mt-8">
              <button onClick={onBack} className="text-sm text-[#636366] border border-[#E5E3DF] px-6 py-3 rounded-lg hover:border-[#636366] hover:text-[#1C1C1E] transition-all">Back</button>
              <button onClick={() => setShowIntro(false)}
                className="group inline-flex items-center gap-2 bg-[#1C1C1E] text-white text-sm font-medium px-8 py-3 rounded-lg hover:bg-[#3A3A3C] transition-all">
                Begin this module
                <svg className="w-4 h-4 transition-transform group-hover:translate-x-1" viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </button>
            </div>
          </motion.div>
        </div>
    ...
    <div className="min-h-screen py-12 px-6 max-w-3xl mx-auto pb-28">
      ...
      <div className="flex gap-1 mb-10">
        {module.steps.map((_, i) => (
          <div key={i} className={['h-0.5 rounded-full flex-1 transition-all duration-300',
      ...
      <AnimatePresence mode="wait" custom={direction}>
        <motion.div key={current} custom={direction} variants={variants}
          ...
          <h2 className="font-serif text-[1.65rem] leading-[1.25] text-[#1C1C1E] mb-3">{step.question}</h2>
          {step.hint && <p className="text-sm text-[#636366] font-light mb-6 leading-relaxed">{step.hint}</p>}
